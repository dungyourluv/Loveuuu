...
function typeText(element, text) {
  ...
  subMit.addEventListener("click", function () {
    if (index >= text.length) {
      ...
      ).innerText = `tớ cũng thích cậu nhiều lắm ${YourName} <33`;
      document.querySelector(
        ".submit-content > p:nth-child(2)"
      ).innerText = `yêu ${YourName} 3000 <3333`;
      document.querySelector(".btn-submit").onclick = function () {
        window.location.href = "https://www.messenger.com/teee.dunn";
      };
    } else {
...
function popClick() {
  ...
  if (name.length < 2) {
    ...
    alert(`hiiii, ${YourName}`);
    document.querySelector(
      ".heading"
    ).innerHTML = `Chào ${YourName} yêu dấu, ${YourName} làm người yêu tớ nhé!`;
    document.querySelector(
      ".sub-heading"
    ).innerHTML = `${YourName} k yêu tớ tớ oánh chết mọe nhé!`;
    document.querySelector(
      ".heading.love"
